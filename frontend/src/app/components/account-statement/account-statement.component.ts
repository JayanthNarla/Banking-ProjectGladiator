...
export class AccountStatementComponent implements OnInit {
  ...
  constructor(
    ...
    private ser: AccountStatementService
  ) {
    this.account = new Account();
    console.log(JSON.parse(localStorage.getItem('cust_id'))['value']);

    this.ser
      .getAccountDetails(JSON.parse(localStorage.getItem('cust_id'))['value'])
      .subscribe(
        (data) => (this.account = data),
        (err) => console.log(err.error_messages)
      );
    ...
    this.stateForm = this.formBuilder.group({
      start: new FormControl('', Validators.required),
        ...
        '',
        Validators.compose([Validators.required, CompareDates])
      ),
    });
  }
  ...
  Get() {
    this.ser
      .getTransactions(
        JSON.parse(localStorage.getItem('cust_id'))['value'],
        this.stateForm.get('start').value,
        this.stateForm.get('end').value
      )
      .subscribe(
        (data) => console.log((this.trans = data)),
        (err) => console.log(err)
      );
  }
