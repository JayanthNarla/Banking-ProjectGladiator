...
export class UserProfileComponent implements OnInit {
  ...
  Update(aadhar: any) {
    ...
    this.service.getUserById(this.aadhar).subscribe((data) => {
      ...
      this.userForm.controls['res_address'].setValue(
        this.userUpdate.res_address
      );
      this.userForm.controls['perm_address'].setValue(
        this.userUpdate.perm_address
      );
    });
  }
  PutSubmit(profile: PersonalDetails) {
    ...
    profile.father_name = this.user.father_name;
    ...
    profile.cust_mail = this.user.cust_mail;
