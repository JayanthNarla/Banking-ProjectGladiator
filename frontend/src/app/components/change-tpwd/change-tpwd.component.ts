...
@Component({
  selector: 'app-change-tpwd',
  templateUrl: './change-tpwd.component.html',
  styleUrls: ['./change-tpwd.component.css'],
})
export class ChangeTpwdComponent implements OnInit {
  frgtPwd: FormGroup;
  otp: Otp;
  cust: Customer;
  mailID: string;
  ...
  updatePassword = () => {
    ...
    if (pwd == cpwd) {
      ...
      this.ls.updateTPassword(this.user).subscribe((data) => {
        this.toastr.success(`Transaction Password Updated`);
