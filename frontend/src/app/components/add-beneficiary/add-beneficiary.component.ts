...
export class AddBeneficiaryComponent implements OnInit {
  ...
  error_messages = {
    ben_name: [
      ...
      { type: 'maxLength', message: 'ben_name cannot exceed 50 alphabets' },
    ],
    ben_acc_num: [
      { type: 'required', message: ' Account number is required' },
      { type: 'maxLength', message: ' Account number cannot exceed 15 digits' },
      { type: 'minlength', message: 'ben_acc_num must be 15 digits' },
    ],
    Re_enterAccount_num: [
      { type: 'required', message: 'Account number is required' },
      {
        type: 'maxLength',
        message: ' Account number does not match with the ben_Acc_num',
      },
      { type: 'minlength', message: 'acc_num must be 15 digits' },
      {
        type: 'Valid',
        message:
          'Entered Accountnumber mismatches with the Beneficiary Accountnumber',
      },
    ],
  ...
  constructor(
    ...
    private ser: BeneficiaryService,
    private acc: AccountStatementService
  ) {
    this.ben = new Add_beneficiary();
    this.acc.getAccountDetails('2345').subscribe((d) => (this.useracc = d));
    this.BeneficiaryForm = this.formBuilder.group({
      ben_name: new FormControl(
        '',
        Validators.compose([Validators.required, Validators.maxLength(50)])
      ),
      ben_acc_num: new FormControl(
        '',
        Validators.compose([
          Validators.required,
      ...
      Re_enterAccount_num: new FormControl(
        '',
        Validators.compose([
          Validators.required,
          Account,
  ...
  onSubmit() {
    ...
    if (this.BeneficiaryForm.valid) {
      ...
      this.ser.AddBeneficiary(this.ben).subscribe(
        (data) => {
          this.toastr.success('Beneficiary added successfully');
        },
        (err) => {
          this.toastr.info('Beneficiary already exists');
        }
      );
      this.BeneficiaryForm.reset();
    }
  ...
  checkAcc() {
    if (
      this.BeneficiaryForm.get('ben_acc_num').value == this.useracc.acc_number
    ) {
